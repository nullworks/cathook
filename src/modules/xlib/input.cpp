...
#include "../../util/logging.h"			// Logging is always cool!
...
bool GetXDisplay() {
	...
	if (!xAppDisplay){
		...
		CatLogging("Xlib Input: Recieved XDisplay!");
	}
	return true;
}
...
bool GetXWindow() {
	...
	if (!xAppWindow){
		...
		CatLogging("Xlib Input: Recieved XWindow: 0x0%x", xAppWindow);
	}
	return true;
}
...
void RefreshState() {
	...
	// Get window bounds
	static Window root_return; static int rel_x, rel_y; static unsigned int border, depth, boundsx, boundsy; 
	if (XGetGeometry(xAppDisplay, xAppWindow, &root_return, &rel_x, &rel_y, &boundsx, &boundsy, &border, &depth)) {
		input::bounds = std::makepair(boundsx, boundsy);
	}
	
	// Update mouse position
	static Window child_return; static int root_x, root_y, mousex, mousey; static unsigned int mask_return; 
	if (XQueryPointer(xAppDisplay, xAppWindow, &root_return, &child_return, &root_x, &root_y, &mousex, &mousey, &mask_return)) { // only update the cursor if this returns true
		if (!(tmpx <= 0 || tmpy <= 0 || tmpx >= input_class->boundsx || tmpy >= input_class->boundsy)) { // Clamp positions to the window
			input::mouse = std::makepair(mousex, mousey);			
		}
	...
	for (const auto& : xlibToCatVar) {
		
		// Get the keycode for the key we are looking for...
		static int current_key;
		current_key = XKeysymToKeycode(xAppDisplay, current.first); 
