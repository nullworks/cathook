...
{
    g_pCVar = iface;
    logging::Info(XORSTR("Set interface to 0x%08x"), iface);
}
                                ...
                                int flags /*= 0*/)
{
    ...
    Assert(pName);
    m_pszName       = pName;
    m_pszHelpString = pHelpString ? pHelpString : XORSTR("");
...
CCommand::CCommand()
{
    if (!s_bBuiltBreakSet)
    {
        ...
        CharacterSetBuild(&s_BreakSet, XORSTR("{}()':"));
    }
...
bool CCommand::Tokenize(const char *pCommand, characterset_t *pBreakSet)
{
    ...
    while (bufParse.IsValid() && (m_nArgc < COMMAND_MAX_ARGC))
    {
        ...
        if (m_nArgc == 1)
        {
            ...
            // The StartGet check is to handle this case: XORSTR("foo")bar
...
const char *CCommand::FindArg(const char *pName) const
{
    ...
    for (int i = 1; i < nArgC; i++)
    {
        if (!Q_stricmp(Arg(i), pName))
            return (i + 1) < nArgC ? Arg(i + 1) : XORSTR("");
    }
    return 0;
}
...
void ConCommand::Dispatch(const CCommand &command)
{
    ...
    AssertMsg(0, XORSTR("Encountered ConCommand '%s' without a callback!\n"),
              GetName());
}
...
void ConVar::InternalSetValue(const char *value)
{
    ...
    {
        Q_snprintf(tempVal, sizeof(tempVal), XORSTR("%f"), fNewValue);
        val = tempVal;
    }
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
    ...
    if (V_strcmp(pszOldValue, m_pszString) != 0)
    {
        ...
        /* URAN */
        // logging::Info(XORSTR("Calling 0x%08x callbacks"), g_pCVar);
...
void ConVar::InternalSetFloatValue(float fNewValue)
{
    ...
    {
        char tempVal[32];
        snprintf(tempVal, sizeof(tempVal), XORSTR("%f"), m_fValue);
...
void ConVar::InternalSetIntValue(int nValue)
{
    ...
    {
        char tempVal[32];
        snprintf(tempVal, sizeof(tempVal), XORSTR("%d"), m_nValue);
        ChangeStringValue(tempVal, flOldValue);
    }
    else
    {
...
{
    m_pszDefaultValue = pszDefault ? pszDefault : XORSTR("");
...
{
public:
    CEmptyConVar() : ConVar(XORSTR(""), XORSTR("0"))
    {
    }
...
void ConVarRef::Init(const char *pName, bool bIgnoreMissing)
{
    ...
    if (!IsValid())
    {
        ...
        if (g_pCVar || bFirst)
        {
            if (!bIgnoreMissing)
            {
                Warning(XORSTR("ConVarRef %s doesn't point to an existing ConVar\n"),
                        pName);
            }
            bFirst = false;
        }
...
void ConVar_PrintFlags(const ConCommandBase *var)
{
    ...
    {
        ConMsg(XORSTR(" replicated"));
        any = true;
    }

    if (var->IsFlagSet(FCVAR_SERVER_CAN_EXECUTE))
    {
        ConMsg(XORSTR(" server_can_execute"));
        any = true;
    }

    if (var->IsFlagSet(FCVAR_CLIENTCMD_CAN_EXECUTE))
    {
        ConMsg(XORSTR(" clientcmd_can_execute"));
        any = true;
    }
...
void ConVar_PrintDescription(const ConCommandBase *pVar)
{
    ...
    if (!pVar->IsCommand())
    {
        ...
        if (pBounded || var->IsFlagSet(FCVAR_NEVER_AS_STRING))
        {
            ...
            {
                Q_snprintf(tempVal, sizeof(tempVal), XORSTR("%d"), intVal);
            }
            else
            {
                Q_snprintf(tempVal, sizeof(tempVal), XORSTR("%f"), floatVal);
            }
        }
        else
        {
        ...
        if (value)
        {
            ConColorMsg(clr, XORSTR("\"%s\" = \"%s\""), var->GetName(), value);

            if (stricmp(value, var->GetDefault()))
            {
                ConMsg(XORSTR(" ( def. \"%s\" )"), var->GetDefault());
            }
        }

        if (bMin)
        {
            ConMsg(XORSTR(" min. %f"), fMin);
        }
        if (bMax)
        {
            ConMsg(XORSTR(" max. %f"), fMax);
        }
