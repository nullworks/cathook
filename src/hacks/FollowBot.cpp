...
void DoWalking()
{
    ...
    if (roaming && CE_BAD(best_target))
    {
            ...
            g_GlobalVars->curtime - 2.5F > crumb_prune_timeout)
        {
            ...
            for (int i = 0; i < HIGHEST_ENTITY; i++)
            {
                ...
                float scr =
                    4096.0f - g_pLocalPlayer->v_Origin.DistTo(ent->m_vecOrigin);
    ...
    // TODO, setting following_idx causes a crash for an unknown reason, probs
    // to do with autoheal. I created a different var to take its place and
    // prevent the crash but i need to fix the crash with the externed var. For
    // now this works and it will stay like this untill I find a way to fix it
