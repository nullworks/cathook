...
public:
    ...
    EntityType m_Type()
    {
        ...
        if (classid == CL_CLASS(CTFPlayer))
            ...
            ret = ENTITY_BUILDING;
        else if (classid == CL_CLASS(CZombie) || classid == CL_CLASS(CTFTankBoss) || classid == CL_CLASS(CMerasmus) || classid == CL_CLASS(CEyeballBoss) || classid == CL_CLASS(CHeadlessHatman))
            ret = ENTITY_NPC;
        else
            ret = ENTITY_GENERIC;
        return ret;
